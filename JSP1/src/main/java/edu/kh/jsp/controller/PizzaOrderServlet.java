...
/*
 ...
 - POST 방식 요청 : form(POST), JS(ajax/REST API) 
 */
...
public class PizzaOrderServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/* JSP로 요청 위임되는 HttpServletRequest 객체에 임의의 값을 추가 */
		String myName = "백동현";
		
		// Attribute(속성) == 객체의 속성 == 변수(필드)
		req.setAttribute("myName", myName);
//		  ㄴ > private String myName = "myName";
		
		// -------------------------------
		
		// pizza_order.jsp가 응답 화면을 대신 만들어 출력할수 있도록
//				HttpServletRequest, HttpServletResponse 를 요청 위임(forward)
		...
		// RequestDispatcher 객체 작성 - 요청 위임 JSP 지정 + req/resp 객체 발송
	...
	/* 요청 주소가 같아도 method가 GET/POST로 다른 경우 따로 처리하는 코드 작성 기능 */
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.setAttribute("pizzaName", pizzaName); // req.setAttribute("변수명", 전달값);
		req.setAttribute("price", price);
		
		// 요청 위임할 JSP 경로 지정
		String path = "/WEB-INF/views/order_result.jsp";
		
		// 요청 발송자 이용, JSP 저장 + 요청 위임
		RequestDispatcher dispatcher = req.getRequestDispatcher(path);
		dispatcher.forward(req, resp);
