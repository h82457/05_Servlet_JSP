...
//		 ---------------------
		...
		// 3. session scope(접속)
//			ㄴ 클라이언트가 서버에 첫 요청을 할떄 생성 (== 클라이언트가 사이트에 접속함)
		// 지정된 세션 만료 시간 또는 브라우저 종료시까지 유지
		
		//	- session이 유지되는동안 어디셔든 사용 가능

		// 1) Session 객체 얻어오기
		HttpSession session = req.getSession();
	 
		// 2) Sesstion 객체에 속성 추가
		session.setAttribute("sessionMessage", "session scope 입니다.");
		
		
//		 ---------------------
		
		// 4. appplication scope
//			-앱 어플리케이션(서버) 당 1개만 존재
//			- 서버 시작시 생성, 종료시 소멸 (종료 전까지 어디서든지 사용 가능)
...
//		 ---------------------	
		...
		// 5. 범위 별 우선 순위 확인 : page > request > session> application
//				ㄴ범위가 좁을수록 우선순위 ↗
