...
<%-- JSP 라이브러리를 현재 JSP에서 사용하겠다는 JSP 지시자 작성 --%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!-- 				ㄴ> 태그 앞에 붙는 단어 ㄴ> core : if, for문 등.. -->
...
<body>
  ...
  <form action="/todo/add"> <!-- method 생략시 기본값인 GET 으로 해석  -->
    <h4> 할 일 추가 </h4>
    <div>
	...
	<table border="1" style="border-collapse: collapse;">
		...
		<tbody>
			...
			items : 순차접근할 List, 배열 / var : 하나씩 꺼내서 저장할 변수 이름 지정 
...
<c:if test="${not empty sessionScope.message}">
	<!-- JSP에서 Java(EL,JSTL), Front(HTML, CSS, JS) 중 Java가 해석 우선 순위가 높다! (먼저 해석)-->
	<script>
		// EL을 이용해서 String을 출력할때 꼭 양쪽에 "" 추가하기!
		const message = "${message}"; 
		alert(message);
	</script>
	
	<%-- sessionScope.message를 한번만 출력한 후 제거하기 --%>
